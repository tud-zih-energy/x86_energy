...
enum x86_energy_granularity
{
    ...
    X86_ENERGY_GRANULARITY_THREAD, /**< The node represents a hardware thread (each core holds at
                                      least one), or a Linux CPU  */
    X86_ENERGY_GRANULARITY_DEVICE, /** < The node represents a device (not used yet) */

    /** Non-ABI, always compare to >= X86_ENERGY_GRANULARITY_SIZE instead of == , since this might
       change */
...
struct x86_energy_architecture_node
{
    ...
    x86_energy_architecture_node_t* children; /**< the children, each system will have a number of
                                                 sockets, ... */
};
...
/**
 *  Free a tree representation of the current hardware that was created with
 * x86_energy_init_architecture_nodes
 ...
 *
 * @param root the tree gathered with x86_energy_init_architecture_nodes
 * @param granularity the granularity to search for (e.g. find module for CPU 15 ->
 * X86_ENERGY_GRANULARITY_MODULE)
 * @param cpu the CPU to ssearch the info for
 * @return the node with the given granularity that is in the graph between root and the cpu with id
 * cpu
 */
...
{
    char* name; /** < Will tell you which mechanism is accessible (e.g. "Intel RAPL"), each
                   mechanism has different implementations for access (sources), e.g. Likwid, sysfs
                   */
    enum x86_energy_granularity
        source_granularities[X86_ENERGY_COUNTER_SIZE]; /**< will tell you the granularity for each
                                                          possible counter, if it is >=
                                                          X86_ENERGY_GRANULARITY_SIZE, it is not
                                                          available */
    size_t nr_avail_sources;                           /** < length of avail_sources */
    struct x86_energy_access_source* avail_sources;    /** < holds a list of possible interfaces to
                                                          access your mechanism, just try them ;) */
...
typedef struct x86_energy_access_source
{
    ...
    x86_energy_single_counter_t (*setup)(enum x86_energy_counter,
                                         size_t index); /**< Call this for adding a counter, will
                                                           return NULL on error */
    double (*read)(x86_energy_single_counter_t t); /**< Read the current energy value in Joules,
                                                      will return < 0.0 on error */
